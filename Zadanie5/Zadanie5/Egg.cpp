...
Egg::Egg(unsigned N, const point3d cords, float scale)
    : m_scale_val{scale}, m_N{N}, m_dist{1.f / m_N} {
...
void Egg::material() { m_material.set_up_material(); }

void Egg::texture() { m_texture.set_up_texture(); }
...
float Egg::fxu(float u, float v) {
  return (-450 * pow(u, 4) + 900 * pow(u, 3) - 810 * pow(u, 2) + 360 * u - 45) *
         cos(M_PI * v);
}

float Egg::fxv(float u, float v) {
  return M_PI *
         (90 * pow(u, 5) - 225 * pow(u, 4) + 270 * pow(u, 3) - 180 * pow(u, 2) +
          45 * u) *
         sin(M_PI * v);
}
...
float Egg::fyv(float u, float v) { return 0.0f; }
...
float Egg::length(point3d** arr, int i, int j) {
  return sqrt((arr[i][j][0] * arr[i][j][0] + arr[i][j][1] * arr[i][j][1] +
               arr[i][j][2] * arr[i][j][2]));
}
...
void Egg::generate_points() {
  ...
  for (int u{0}; u < m_N; u++) {
    ...
    for (int v{0}; v < m_N; v++) {
      float p = ((-90) * powf(u_off, 5) + 225 * powf(u_off, 4) -
                 270 * powf(u_off, 3) + 180 * powf(u_off, 2) - 45 * u_off);

      m_points[u][v][0] = p * cos(M_PI * v_off);
      m_points[u][v][1] =
          160 * powf(u_off, 4) - 320 * powf(u_off, 3) + 160 * powf(u_off, 2);
...
float Egg::rand_color() { return rand() % 1000 / 1000.; }
...
void Egg::set_texture(Texture text) {
  m_texture = text;
}
